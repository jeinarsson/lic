...
Finally, for larger tri-axial particles both fluid and particle inertia contribute to the dynamics. In Paper~B (see \Secref{inertia}) we compute the first effects of particle inertia ($\st>0$) on the dynamics of an axisymmetric particle. The corresponding calculation for a tri-axial particle remains as an open question. Fluid inertia ($\rep > 0$) is more difficult, even in the case of axisymmetric particles (see \Secref{inertiaoutlook}).
