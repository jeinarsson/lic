...
#				plot_mean_and_deviation_nonlog(axes[0], times, data)
	...
	axes[0].set_ylabel(r'$\langle \mathrm{Tr} \mathbb{S}_0\mathbb{O}_0\mathbb{S}_t\rangle\tau^3_{K}$')
	axes[1].set_ylabel(r"$\int_{-\infty}^t\!\!\!\mathrm{d} t' \langle \mathrm{Tr} \mathbb{S}_0\mathbb{O}_0\mathbb{S}_{t'}\rangle\tau^3_{K} $")
	...
	if not exists("output/"):
		mkdir("output")
	pp = matplotlib.backends.backend_pdf.PdfPages('output/sosintegral.pdf')
	pp.savefig(fig1)
	pp.close()
