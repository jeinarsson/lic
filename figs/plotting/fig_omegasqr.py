...
def main():
	...
	for dataset in [output_file['omegasqr']]:
		pdf = dataset[0,:]
	...
	ax.set_xlabel(r'$\dot n^2 \tau_{K}$')
	ax.set_ylabel(r'$P(\dot n^2 \tau_{K})$',rotation=90)
	#ax.set_ylim(0,1)
	#ax1.set_xlim(0,2)
	...
	if not exists("output/"):
		mkdir("output")
	pp = matplotlib.backends.backend_pdf.PdfPages('output/omegasqr_pdf.pdf')
	pp.savefig(fig1)
	pp.close()
